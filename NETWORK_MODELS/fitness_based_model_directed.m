function W = fitness_based_model_directed(n, p, fitness_func, weight_func)
        ...
        for j = 1:n
            if i ~= j  % Avoid self-loops
                ...
                if rand < p * fitness(i) * fitness(j)
                    ...
                    W(i, j) = weight;
                    % Note: W(j, i) is not set, making the network directed
                end
            end
        end
    end
...
% Example usage:
% W = fitness_based_model_directed(100, 0.1, @(n) rand(1, n), @() exprnd(1));
...
% The resulting adjacency matrix W is not symmetric, representing
% directed edges. W(i,j) represents an edge from node i to node j.